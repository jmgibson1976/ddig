...
class TestFetchParsesCsv:
    def test_yields_domain_objects(self, tmp_path):
        ...
        with patch("ddig.sources.majestic.DATA_DIR", tmp_path), \
             patch.object(src._session, "get", return_value=resp), \
    ...
    def test_domain_fqdn_correct(self, tmp_path):
        src  = MajesticMillionSource()
        resp = _mock_streaming_response(SAMPLE_CSV)
        with patch("ddig.sources.majestic.DATA_DIR", tmp_path), \
             patch.object(src._session, "get", return_value=resp), \
    ...
    def test_domain_name_correct(self, tmp_path):
        src  = MajesticMillionSource()
        resp = _mock_streaming_response(SAMPLE_CSV)
        with patch("ddig.sources.majestic.DATA_DIR", tmp_path), \
             patch.object(src._session, "get", return_value=resp), \
    ...
    def test_fqdn_not_doubled(self, tmp_path):
        csv_data = (
            "GlobalRank,TldRank,Domain,TLD,RefSubNets,RefIPs\n"
    ...
    def test_domain_name_lowercased(self, tmp_path):
    ...
    def test_domain_tld_correct(self, tmp_path):
        src  = MajesticMillionSource()
        resp = _mock_streaming_response(SAMPLE_CSV)
        with patch("ddig.sources.majestic.DATA_DIR", tmp_path), \
             patch.object(src._session, "get", return_value=resp), \
    ...
    def test_domain_source_is_majestic(self, tmp_path):
        src  = MajesticMillionSource()
        resp = _mock_streaming_response(SAMPLE_CSV)
        with patch("ddig.sources.majestic.DATA_DIR", tmp_path), \
             patch.object(src._session, "get", return_value=resp), \
    ...
    def test_rank_populated(self, tmp_path):
        src  = MajesticMillionSource()
        resp = _mock_streaming_response(SAMPLE_CSV)
        with patch("ddig.sources.majestic.DATA_DIR", tmp_path), \
             patch.object(src._session, "get", return_value=resp), \
    ...
    def test_rank_increments(self, tmp_path):
        src  = MajesticMillionSource()
        resp = _mock_streaming_response(SAMPLE_CSV)
        with patch("ddig.sources.majestic.DATA_DIR", tmp_path), \
             patch.object(src._session, "get", return_value=resp), \
    ...
    def test_fetched_at_is_timezone_aware(self, tmp_path):
        src  = MajesticMillionSource()
        resp = _mock_streaming_response(SAMPLE_CSV)
        with patch("ddig.sources.majestic.DATA_DIR", tmp_path), \
             patch.object(src._session, "get", return_value=resp), \
...
class TestFetchHandlesMalformedRows:
    ...
    def test_skips_empty_tld(self, tmp_path):
        src  = MajesticMillionSource()
        resp = _mock_streaming_response(MALFORMED_CSV)
        with patch("ddig.sources.majestic.DATA_DIR", tmp_path), \
             patch.object(src._session, "get", return_value=resp), \
    ...
    def test_invalid_ref_subnets_defaults_to_zero(self, tmp_path):
        src  = MajesticMillionSource()
        resp = _mock_streaming_response(MALFORMED_CSV)
        with patch("ddig.sources.majestic.DATA_DIR", tmp_path), \
             patch.object(src._session, "get", return_value=resp), \
    ...
    def test_valid_rows_still_yielded(self, tmp_path):
        src  = MajesticMillionSource()
        resp = _mock_streaming_response(MALFORMED_CSV)
        with patch("ddig.sources.majestic.DATA_DIR", tmp_path), \
             patch.object(src._session, "get", return_value=resp), \
...
class TestFetchLimit:
    def test_limit_stops_at_n(self, tmp_path):
    ...
    def test_limit_larger_than_data_returns_all(self, tmp_path):
...
class TestFetchRankFilters:
    def test_max_rank_filters_lower_ranked(self, tmp_path):
    ...
    def test_min_rank_filters_higher_ranked(self, tmp_path):
    ...
    def test_min_and_max_rank_combined(self, tmp_path):
    ...
    def test_min_rank_zero_no_lower_bound(self, tmp_path):
    ...
    def test_max_rank_zero_no_upper_bound(self, tmp_path):
...
class TestFetchHttpErrors:
    ...
    def test_get_called_with_correct_url(self, tmp_path):
        src  = MajesticMillionSource()
        resp = _mock_streaming_response(SAMPLE_CSV)
        with patch("ddig.sources.majestic.DATA_DIR", tmp_path), \
             patch.object(src._session, "get", return_value=resp) as mock_get, \
             _patch_store(SAMPLE_FQDNS):
            list(src.fetch())
    ...
    def test_get_called_with_custom_url(self, tmp_path):
...
# ------------------------------------------------------------------ #
# Caching (_get_lines / _local_path)                                  #
# ------------------------------------------------------------------ #

class TestCaching:
    def test_data_dir_created_on_init(self, tmp_path):
        with patch("ddig.sources.majestic.DATA_DIR", tmp_path / "majestic"):
            MajesticMillionSource()
            assert (tmp_path / "majestic").exists()
    ...
    def test_downloads_and_saves_when_no_cache(self, tmp_path):
        ...
        with patch("ddig.sources.majestic.DATA_DIR", tmp_path), \
             ...
             _patch_store(SAMPLE_FQDNS):
            list(src.fetch())
        saved = list(tmp_path.glob("majestic_million_*.csv"))
        assert len(saved) == 1
    ...
    def test_uses_cached_file_skips_download(self, tmp_path):
        ...
        cached   = tmp_path / f"majestic_million_{today}.csv"
    ...
    def test_cache_filename_includes_today(self, tmp_path):
        src  = MajesticMillionSource()
        resp = _mock_streaming_response(SAMPLE_CSV)
        today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
        with patch("ddig.sources.majestic.DATA_DIR", tmp_path), \
            ...
            list(src.fetch())
        saved = list(tmp_path.glob("majestic_million_*.csv"))
        assert saved[0].name == f"majestic_million_{today}.csv"