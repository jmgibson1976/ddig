...
class TestNameSourceCaching:
    def test_data_dir_created_on_init(self, tmp_path):
        with patch("ddig.sources.name.DATA_DIR", tmp_path / "name"):
            with patch("ddig.env._cache", ENV_PATCH):
                NameSource()
    ...
    def test_cached_file_skips_network(self, tmp_path):
        ...
        cache_file = tmp_path / f"name_{today}.csv"
    ...
    def test_downloads_and_saves_when_no_cache(self, tmp_path):
        ...
        with patch("ddig.env._cache", ENV_PATCH), \
            ...
            MockSession.return_value.get.return_value = resp
            list(src.fetch())
        saved = list(tmp_path.glob("name_*.csv"))
        assert len(saved) == 1
    ...
    def test_cache_filename_includes_today(self, tmp_path):
        ...
        with patch("ddig.env._cache", ENV_PATCH), \
            ...
            list(src.fetch())
        saved = list(tmp_path.glob("name_*.csv"))
        assert saved[0].name == f"name_{today}.csv"
