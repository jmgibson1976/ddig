...
| Command | Description |
|---------|-------------|
| `ddig --version` | Print the installed ddig version and exit |
...
Options:
  -s, --source TEXT       Source to fetch from: dropcatch | expireddomains | czds | majestic | name | snapnames | parkio
                          [default: dropcatch]
  ...
  -p, --pages INT         Max pages to scrape per TLD (expireddomains, parkio)  [default: 10/20]
...
```bash
# SnapNames — no credentials, fetches allexpiring + deleting feeds by default
ddig fetch --source snapnames --score
ddig fetch --source snapnames --feed allexpiring --verbose
ddig fetch --source snapnames --feed deleting --verbose
```

```bash
# Park.io — no credentials, all 18 TLDs by default
ddig fetch --source parkio --score
ddig fetch --source parkio --tlds io,co,me --verbose
```
...
Options:
  ...
  -t, --tld TEXT          Filter by TLD e.g. com, io, app
      --source TEXT       Filter by source: dropcatch | expireddomains | czds | majestic | name | snapnames | parkio
...
| `snapnames`      | None                             | ~213K/day (2 feeds)          | ✅ Daily             | [docs/sources/snapnames.md](docs/sources/snapnames.md)           |
| `parkio`         | None                             | Up to 72K (18 TLDs × 20 pages) | ✅ Daily             | [docs/sources/parkio.md](docs/sources/parkio.md)                 |
| `nrd` *(purge)*  | None                             | ~100K–200K/day (filter only) | ✅ Daily             | [docs/sources/nrd.md](docs/sources/nrd.md)                       |
...
│   │   ├── snapnames.py       # SnapNames CSV feeds (no auth)
│   │   ├── parkio.py        # Park.io JSON API (no auth, 18 TLDs)
│   │   └── nrd.py           # NRD feeds — newly registered domains (purge only)
│   ├── storage/
