...
from .sources.snapnames import SnapNamesSource
from .sources.parkio import ParkIOSource, ALL_TLDS as PARKIO_TLDS, DEFAULT_MAX_PAGES
from .sources.nrd import NRDSource
...
def _version_callback(value: bool) -> None:
    if value:
        try:
            typer.echo(importlib.metadata.version("ddig"))
...
def _main_callback(
    version: bool = typer.Option(
        ...
        is_eager=True,
        help="Show the installed ddig version and exit.",
    ),
) -> None:
    pass
...
@app.command()
def fetch(
    source:   str            = typer.Option(...,   "--source",   "-s", help="dropcatch | expireddomains | czds | majestic | name | snapnames | parkio"),
    feed:     Optional[str]  = typer.Option(None,  "--feed",     "-f", help="dropcatch: dropping-today | …  expireddomains: deleted | …  snapnames: allexpiring | deleting | <filename stem>"),
    ...
    if source == "dropcatch":
    ...
    elif source == "snapnames":
        src = SnapNamesSource(feed=feed)
        ...
        if tlds:
            ...
            tld_list = [tld.strip().lstrip(".")]
        src = ParkIOSource(tlds=tld_list, max_pages=pages if pages != 10 else DEFAULT_MAX_PAGES)

    else:
        console.print(f"[red]Unknown source: {source!r}[/red]")
        console.print("Valid sources: dropcatch, expireddomains, czds, majestic, name, snapnames, parkio")
        raise typer.Exit(1)
...
def search(
    ...
    source:        Optional[str]   = typer.Option(None,    "--source",        "-s",  help="Filter by source: dropcatch | expireddomains | czds | majestic | name"),
...
@app.command()
def purge_registered(
    source:  str  = typer.Option("all",           "--source",  "-s", help="nrd | whoisds | all"),
    dry_run: bool = typer.Option(False,           "--dry-run",       help="Show matches without deleting"),
    db:      Path = typer.Option(DEFAULT_DB_PATH, "--db"),
    verbose: bool = typer.Option(False,           "--verbose", "-v"),
) -> None:
    """Remove newly-registered domains from the database using NRD feeds."""
    import time
    _setup_logging(verbose)

    valid_sources = {"nrd", "whoisds", "all"}
    if source not in valid_sources:
        console.print(f"[red]Unknown --source: {source!r}. Valid: nrd | whoisds | all[/red]")
        raise typer.Exit(1)

    feed_list = ["nrd", "whoisds"] if source == "all" else [source]
    store     = DomainStore(db_path=db)

    t0  = time.perf_counter()
    src = NRDSource(sources=feed_list)
    with console.status("[bold green]Fetching newly registered domains…"):
        registered = set(src.fetch())
    t_fetch = time.perf_counter() - t0
    console.print(
        f"[cyan]Fetched[/cyan] {len(registered):,} newly registered domains  "
        f"[dim]({t_fetch:.1f}s)[/dim]"
    )

    if not registered:
        console.print("[yellow]No newly registered domains found — nothing to purge.[/yellow]")
        raise typer.Exit(0)

    with console.status("[bold green]Checking database…"):
        db_fqdns    = store.get_all_fqdns()
        matched     = registered & db_fqdns
        watchlisted = store.get_watchlist_fqdns()
        protected   = matched & watchlisted
        to_delete   = matched - watchlisted

    console.print(f"Matched [bold]{len(matched):,}[/bold] domains in your database")

    if protected:
        console.print(f"[yellow]Skipping {len(protected):,} watchlisted domain(s)[/yellow]")

    if not to_delete:
        console.print("[green]✓[/green] Nothing to delete.")
        raise typer.Exit(0)
    ...
    if dry_run:
        console.print(
            f"[yellow]DRY RUN[/yellow] — would delete [bold]{len(to_delete):,}[/bold] domains. "
            f"Run without [bold]--dry-run[/bold] to purge."
        )
        sample = sorted(to_delete)[:20]
        for fqdn in sample:
            console.print(f"  [dim]{fqdn}[/dim]")
        if len(to_delete) > 20:
            console.print(f"  [dim]… and {len(to_delete) - 20:,} more[/dim]")
        raise typer.Exit(0)

    t0 = time.perf_counter()
    with console.status("[bold green]Purging registered domains…"):
        deleted = store.purge_fqdns(to_delete, skip=watchlisted)
    t_purge = time.perf_counter() - t0
    console.print(
        f"[green]✓[/green] Deleted [bold]{deleted:,}[/bold] domains  "
        f"[dim]({t_purge:.1f}s)[/dim]"
    )
